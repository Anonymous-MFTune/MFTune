...
class Logger:
    ...
    def logging_data(self, config, perf, evaluated_cost, fidelity, log_path, log_file):
        ...
        with open(log_file, 'a', newline='') as file:
            writer = csv.writer(file)
            factors = list(fidelity.values())
            row = [config[param] for param in self.target_system.knobs_info.keys()] + [perf, evaluated_cost, factors]
            writer.writerow(row)

    def logging_data2(self, config, metrics, fidelity, log_path, log_file):
        """
        Record the config and associated metrics + fidelity values into a CSV log file.

        :param config: Dict[str, Any] - configuration knob values
        :param metrics: Dict[str, Any] - performance/cost info
        :param fidelity: Dict[str, Any] - fidelity factors
        """
        log_file_path = os.path.join(log_path, log_file)
        os.makedirs(log_path, exist_ok=True)

        # Build headers and data row dynamically
        knob_keys = list(self.target_system.knobs_info.keys())
        metric_keys = list(metrics.keys())
        fidelity_keys = list(fidelity.keys())

        headers = knob_keys + metric_keys + ['fidelity']
        row = [config.get(k, "") for k in knob_keys] + \
              [metrics.get(m, "") for m in metric_keys] + \
    ...
    def logging_cyber_twin(self, config, perf, evaluated_cost, fidelity, cyber_twin_path, cyber_twin_file):
