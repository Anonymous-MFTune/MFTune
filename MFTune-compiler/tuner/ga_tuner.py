...
class GATuner:
    ...
    def tune_ga(self):
        ...
        if self.fidelity_type == 'single_fidelity':
            ...
            print(f"[DEBUG] Predefined Budget for LFLHS: {budget_4_lflhs} | Fidelity Id: {fidelity_id} | Num of Sampled Configs: {num_configs}")
    ...
    def evaluate_config_pop(self, configs, factors, stage_budget=None, reset_container: Optional[bool] = None):


        # check the environment
        # --- initialization  ---
        if not hasattr(self, "reset_every_n_single"):
            self.reset_every_n_single = 50
        if not hasattr(self, "_single_eval_since_reset"):
            self._single_eval_since_reset = 0

        # --- analyse whether we need to reset container ---
        if reset_container is None:
            if len(configs) > 1:
                do_reset = True
                self._single_eval_since_reset = 0
            else:  # len(configs) == 1
                do_reset = (self._single_eval_since_reset % self.reset_every_n_single == 0)
        else:
            do_reset = bool(reset_container)
            if do_reset:
                self._single_eval_since_reset = 0
        ...
        if do_reset:
            try:
                ...
                print(f'Container stopped for reset. {self._single_eval_since_reset}')
            except Exception as e:
                print(f"WARN: restart container failed: {e}")
        else:
            if not self.target_system.start():
                print("WARN: Container was not running; started it without a full reset.")
        ...
        for idx, config in enumerate(configs):
            if len(configs) == 1 and idx == 0:
                self._single_eval_since_reset += 1
