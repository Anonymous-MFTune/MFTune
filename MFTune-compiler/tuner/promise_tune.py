...
class PromiseTuner:
    ...
    def config_in_rule(self, config, rule, feature_names):
        ...
        for key, threshold, direction in rule:
            ...
            if knobs_info[key]['type'] == 'enum':
                val = knobs_info[key]['enum_values'].index(val)  # encode categorical value into corresponding index
    ...
    def purify_rules_by_causality(self, X, y, rules):
        ...
        """
        # 1. construcet DataFrame: each column is a rule_feature, last column is performance
        df = pd.DataFrame(X, columns=[f'rule_{i}' for i in range(len(rules))])
        df['perf'] = y
        data = df.to_numpy()
        num_features = data.shape[1]

        # 2. initialize causal structure using FCI
        try:
            ...
            adjacency_matrix[:, -1] = 0  # make sure performance without outgoing edge
        except Exception as e:
        ...
        prior_knowledge[-1, :-1] = 1  # perf can be influenced by all rule
        prior_knowledge[-1, -1] = -1  # perf can not influence the other variable

        # 4. fit causal model using DirectLiNGAM, compute ACE for each rule
        purified = []
        try:
