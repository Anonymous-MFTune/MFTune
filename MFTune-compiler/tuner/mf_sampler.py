...
class MFSampler:
    ...
    def sampling_and_evaluate(self):
        ...
        for fidelity in fidelities:
            ...
            for config in configs:
                ...
                run_time, compile_time, exe_size, csmith_time, source_size, source_lines = self.workload_controller.run_workload(fidelity, config)
                evaluated_time = time.time() - start_time
                self.logging_data(log_file_path, config, run_time, compile_time, evaluated_time)
    ...
    def logging_data(self, log_file_path, config, run_time, compile_time, evaluated_time):
        """recording configs and corresponding perfs """
        if not os.path.exists(log_file_path):
            with open(log_file_path, 'w', newline='') as file:
                writer = csv.writer(file)
                header = list(self.target_system.knobs_info.keys()) + ['run_time', 'compile_time', 'evaluated_time']
                writer.writerow(header)

        with open(log_file_path, 'a', newline='') as file:
            writer = csv.writer(file)
            row = [config[param] for param in self.target_system.knobs_info.keys()] + [run_time, compile_time, evaluated_time]
            writer.writerow(row)

    @staticmethod
    def generate_data_file_name(factors):
        """Generate file name using key-value pairs for better extensibility."""
        parts = [f"{key}_{value}" for key, value in factors.items()]
