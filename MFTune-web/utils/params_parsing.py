...
def parse_args(file):
    ...
    global knob_config
    f = open(config_dict['server']['knob_config_file'])
    knob_config = json.load(f)

    return get_default_dict(config_dict["server"]), get_default_dict(config_dict['workload']), get_default_dict(config_dict['tune'])
