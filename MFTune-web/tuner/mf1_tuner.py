...
class MF1Tuner:
    ...
    def tune_mf1(self):
        ...
        if self.fidelity_type == 'single_fidelity':
            ...
            lf_factors = self.rnd_factors
            lf_corr = 0  # placeholder
            lf_cost = 30  # placeholder
            print(f"FIDELITY SETTING: {lf_factors} | CORRELATION: {lf_corr}(placeholder) | COST: {lf_cost}(placeholder)")

            print("=========== CONFIGURATION SPACE ANALYSIS ==========")
            budget_4_hfea = self.total_budget * 0.5  # preserve 50% budget for the last high-fidelity EA
