...
class MFSampler:
    ...
    def sampling_and_evaluate(self):
        ...
        for fidelity in fidelities:
            ...
            for config in configs:
                ...
                if ready:
                    RPS, TPR = self.workload_controller.run_workload(fidelity)
                else:
                    RPS, TPR = 0, 0
                evaluated_time = time.time() - start_time
                self.logging_data(log_file_path, config, RPS, TPR, evaluated_time)
    ...
    def logging_data(self, log_file_path, config, RPS, TPR, evaluated_time):
        """recording configs and corresponding perfs """
        if not os.path.exists(log_file_path):
            with open(log_file_path, 'w', newline='') as file:
                writer = csv.writer(file)
                header = list(self.target_system.knobs_info.keys()) + ['RPS', 'TPR', 'evaluated_time']
                writer.writerow(header)

        with open(log_file_path, 'a', newline='') as file:
            writer = csv.writer(file)
            row = [config[param] for param in self.target_system.knobs_info.keys()] + [RPS, TPR, evaluated_time]
            writer.writerow(row)
