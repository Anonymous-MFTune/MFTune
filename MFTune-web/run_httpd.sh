...
#declare -a methods=("bestconfig" "hebo" "bohb"  "dehb")
#declare -a services=("httpdbestconfig" "httpdhebo" "httpdbohb" "httpddehb")
#declare -a ports=("9081" "9088" "9089" "9090")
#declare -a fidelity_types=("single_fidelity" "single_fidelity" "multi_fidelity" "multi_fidelity")

declare -a methods=("priorband" "promise")
declare -a services=("httpdpriorband" "httpdpromise")
declare -a ports=("9092" "9091")
...
for i in "${!methods[@]}"; do
  ...
  for run in {0..9}; do
    ...
    SERVER_PORT=$port \
    RUN=$run \
    docker-compose up -d app_tuning $server_service >> "$run_log" 2>&1
