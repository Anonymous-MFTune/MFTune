...
def main():
    ...
    parser.add_argument('--server_port', type=str, required=False, help='server port for docker connection')
    ...
    args_server, args_workload, args_tune = parse_args(opt.config)
    ...
    args_server['container_name'] = opt.container_name
    # if we use docker network, directly using service name to get connection
    if args_server['server'] == 'tomcat':
        args_server['url'] = f"http://{opt.service_name}:8080/tomcatsampling/test.html"
    else:
        args_server['url'] = f"http://{opt.service_name}:80/test.html"
    ...
    if args_tune['tuning_method'] == 'ga':
        optimizer = GATuner(args_server, args_workload, args_tune, run)
        optimizer.tune_ga()
    elif args_tune['tuning_method'] == 'mf_analyser':
        analyser = MFAnalyser(args_server, args_workload, args_tune, run)
        analyser.analyse_and_verify_configs()
    elif args_tune['tuning_method'] == 'mf_sampler':
        sampler = MFSampler(args_server, args_workload, args_tune, run)
        sampler.sampling_and_evaluate()
    elif args_tune['tuning_method'] == 'flash':
        optimizer = FLASHTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_flash()
    elif args_tune['tuning_method'] == 'bestconfig':
        optimizer = BestConfigTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_bestconfig()
    elif args_tune['tuning_method'] == 'smac':
        optimizer = SMACTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_smac()
    elif args_tune['tuning_method'] == 'hyperband':
        optimizer = HBTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_hyperband()
    elif args_tune['tuning_method'] == 'bohb':
        optimizer = BOHBTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_bohb()
    elif args_tune['tuning_method'] == 'dehb':
        optimizer = DEHBTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_dehb()
    elif args_tune['tuning_method'] == 'hebo':
        optimizer = HEBOTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_hebo()
    elif args_tune['tuning_method'] == 'promise':
        optimizer = PromiseTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_promise()
    elif args_tune['tuning_method'] == 'priorband':
        optimizer = PriorBandTuner(args_server, args_workload, args_tune, run)
        optimizer.tune_priorband()
    elif args_tune['tuning_method'] == 'mf1':
        optimizer = MF1Tuner(args_server, args_workload, args_tune, run)
        optimizer.tune_mf1()
    elif args_tune['tuning_method'] == 'mf2':
        optimizer = MF2Tuner(args_server, args_workload, args_tune, run)
        optimizer.tune_mf2()
