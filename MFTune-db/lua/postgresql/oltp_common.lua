...
local function get_id()
   local thread_id = sysbench.tid -- current ID
   local thread_count = sysbench.opt.threads -- total threads
   local id_per_thread = math.ceil(sysbench.opt.table_size / thread_count)

   local start_id = id_per_thread * thread_id + 1
   local end_id = start_id + id_per_thread - 1


   return sysbench.rand.uniform(start_id, math.min(end_id, sysbench.opt.table_size))
end
